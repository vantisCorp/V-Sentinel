...
impl Hypervisor {
    ...
    pub async fn initialize(&self) -> Result<()> {
        ...
        *self.svm_enabled.write().await = svm_supported;
        ...
        if !vmx_supported && !svm_supported {
            return Err(anyhow::anyhow!("No hardware virtualization support detected"));
        }
        ...
        *self.initialized.write().await = true;
        ...
        info!("VMX: {}, SVM: {}, EPT: {}, VPID: {}", 
            vmx_supported, svm_supported, *self.ept_enabled.read().await, *self.vpid_enabled.read().await);
    ...
    pub async fn inject_interrupt(&self, vm_id: usize, vector: u8) -> Result<()> {
        ...
        if vm.state != VMState::Running {
            return Err(anyhow::anyhow!("Cannot inject interrupt into VM in state {:?}", vm.state));
        }
    ...
    pub async fn get_stats(&self) -> HypervisorStats {
        let vms = self.vms.read().await;
        let running_count = vms.values().filter(|vm| vm.state == VMState::Running).count();
...
mod tests {
    ...
    async fn test_vm_lifecycle() {
        ...
        assert_eq!(hypervisor.get_vm_state(vm_id).await.unwrap(), VMState::Paused);
        ...
        assert_eq!(hypervisor.get_vm_state(vm_id).await.unwrap(), VMState::Stopped);
    }
    ...
    async fn test_vm_exit_handling() {
        ...
        assert!(hypervisor.handle_vm_exit(vm_id, VMExitReason::EptViolation).await.is_ok());
        
        // Handle CR access
        assert!(hypervisor.handle_vm_exit(vm_id, VMExitReason::CrAccess).await.is_ok());
        
        // Handle MSR read
        assert!(hypervisor.handle_vm_exit(vm_id, VMExitReason::MsrRead).await.is_ok());
        
        // Handle MSR write
        assert!(hypervisor.handle_vm_exit(vm_id, VMExitReason::MsrWrite).await.is_ok());
        
        // Handle I/O instruction
        assert!(hypervisor.handle_vm_exit(vm_id, VMExitReason::IoInstruction).await.is_ok());
    }
    ...
    async fn test_shutdown_stops_all_vms() {
        ...
        assert_eq!(hypervisor.get_vm_state(vm1).await.unwrap(), VMState::Stopped);
        assert_eq!(hypervisor.get_vm_state(vm2).await.unwrap(), VMState::Stopped);
    }
}
