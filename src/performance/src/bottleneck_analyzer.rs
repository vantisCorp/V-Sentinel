...
impl BottleneckAnalyzer {
    ...
    pub fn profile_operation<F>(&mut self, component: &str, operation: &str, f: F) -> PerformanceProfile
    where
        F: FnOnce(),
    {
    ...
    fn analyze_cache_bottlenecks(&mut self, profile: &PerformanceProfile) {
        if profile.cache_hits + profile.cache_misses > 0 {
            let miss_rate = profile.cache_misses as f64 / (profile.cache_hits + profile.cache_misses) as f64;
    ...
    fn analyze_lock_contention(&mut self, profile: &PerformanceProfile) {
        let lock_ratio = profile.lock_wait_time.as_nanos() as f64 / profile.wall_time.as_nanos() as f64;
        ...
        if lock_ratio > threshold {
            ...
            self.bottlenecks.push(Bottleneck {
                ...
                threshold_value: threshold * 100.0,
                impact: "Lock contention limits parallelism and causes thread starvation".to_string(),
                suggested_optimizations: vec![
    ...
    pub fn generate_optimization_report(&self) -> String {
        ...
        if !critical.is_empty() {
            ...
            for bottleneck in &critical {
                report.push_str(&format!("### {}. {} - {}\n\n", 
                    bottleneck.component, bottleneck.operation, 
        ...
        // Medium bottlenecks
        if !medium.is_empty() {
            report.push_str(&format!("## 🟡 Medium Priority Bottlenecks ({})\n\n", medium.len()));
        ...
        // Low bottlenecks
        if !low.is_empty() {
            report.push_str(&format!("## 🟢 Low Priority Bottlenecks ({})\n\n", low.len()));
        ...
        report.push_str(&format!("- Total Bottlenecks: {}\n", self.bottlenecks.len()));
...
mod tests {
    ...
    fn test_bottleneck_detection() {
        ...
        let profile = analyzer.profile_operation("Test Component", "CPU Intensive Operation", || {
