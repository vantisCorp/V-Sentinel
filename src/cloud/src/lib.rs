...
impl CloudSecurityManager {
    ...
    pub async fn scan_container(&self, image: ContainerImage) -> Result<ContainerScanResult> {
        ...
        info!(
            "Container scan complete: {} vulnerabilities found",
    ...
    pub async fn scan_serverless(
        &self,
        function: ServerlessFunction,
    ) -> Result<ServerlessScanResult> {
        if !*self.active.read().await {
            return Err(anyhow::anyhow!("Cloud Security Manager not active"));
        }
        ...
        info!(
            "Serverless scan complete: {} vulnerabilities found",
            result.vulnerabilities.len()
        );
...
mod tests {
    ...
    async fn test_add_provider() {
        ...
        assert!(manager
            .add_provider(CloudProvider::AWS, config)
            .await
            .is_ok());
    }
