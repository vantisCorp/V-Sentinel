...
impl TrafficAnalyzer {
    ...
    fn add_default_signatures(&mut self) {
        let signatures = vec![
            ...
            TrafficSignature {
                ...
                patterns: vec![
                    r"sk-ant-[a-zA-Z0-9]{20,}".to_string(),
                ],
                confidence: 0.95,
                risk_level: 0.8,
            },
            ...
            TrafficSignature {
                ...
                patterns: vec![
                    r"(?i)ignore\s+(all\s+)?(previous|above)\s+(instructions?|prompts?)".to_string(),
    ...
    pub async fn analyze(&self, data: &[u8]) -> Result<Vec<AITrafficPattern>> {
        ...
        for compiled in &self.patterns {
            if let Some(sig) = self.signatures.iter().find(|s| s.id == compiled.signature_id) {
                for capture in compiled.regex.find_iter(&data_str) {
                    let pattern = AITrafficPattern {
                        ...
                        detected_data: vec![("match".to_string(), serde_json::json!(capture.as_str()))]
                            .into_iter()
                            .collect(),
                        timestamp: Utc::now(),
...
mod tests {
    ...
    async fn test_api_key_detection() {
        ...
        assert!(patterns.iter().any(|p| p.matched_signature == "openai-api-key"));
    }
    ...
    async fn test_prompt_injection_detection() {
        ...
        let patterns = analyzer.analyze(data).await.unwrap();
        assert!(!patterns.is_empty());
        assert!(patterns.iter().any(|p| p.matched_signature == "prompt-injection"));
    }
}